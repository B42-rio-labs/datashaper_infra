...
- `debug_network_exporter` exposes the current Docker containers attached to the `debug` network so Grafana panels can filter cAdvisor metrics by network membership.
- `debug_packet_capture` writes live tcpdump output to Docker logs, which Promtail ships to Loki.
...
The default filter excludes the `debug` network subnet and common monitoring ports so Grafana/Loki/Promtail/Prometheus/cAdvisor traffic does not dominate the packet trace. Grafana dashboard queries also hide internal `debug_*` containers from application panels and expose a `debug_container` selector for choosing which containers to monitor. Use a narrower filter in production-like environments to avoid high log volume.
